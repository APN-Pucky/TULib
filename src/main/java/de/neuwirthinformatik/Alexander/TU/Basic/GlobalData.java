...
public class GlobalData {
	...
	public static String getNameAndLevelByID(int s_id) {
		...
		if (c == null) {
			System.out.println("WTFFFFFFFFFFF!!!!!!!! --1--  " + s_id);
	...
	public static CardInstance getCardInstanceByNameAndLevel(String name) {
		...
		int level = 1;
		try {
			level = Integer.parseInt(split[split.length - 1].trim());
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (c == null)
			System.out.println("WTFFFFFFFFFFF!!!!!!!! --3--  " + name);
	...
	public static Card getCardByID(int s_id) {
		...
		/*
		 * for(Card c : cards) { for(int id : c.getIDs()) { if(id == s_id)return c; } }
		 * return null;
		 */
	}
	...
	public static Mission getMissionByName(String name) {
		for (Mission c : missions) {
			if (c != null && StringUtil.equalsIgnoreSpecial(c.getName(), name))
				return c;
		}
		return null;
	}
	...
	public static String getDeckString(int[] deck) {
		...
		for (int i : deck) {
			if (i != 0)
				decks += GlobalData.getNameAndLevelByID(i) + ", ";
		}
		return decks;
	}
	...
	public static String getInvString(int[] ids) {
		...
		for (int j = 0; j < ids.length;) {
			...
			if (ids[j] != 0)
				inv += GlobalData.getNameAndLevelByID(ids[j]) + "#" + count + "\n";
			j += count;
		}
		return inv;
	}
	...
	public static boolean isDominion(int dom_id) {
		...
		if (d == null) {
			TU.log.w("NULL == DOM ||| UPDATE XMLS?!?! ||| " + dom_id, "DATA");
			return false;
		}
		boolean ret = d.category == CardCategory.DOMINION;
		// if(TUM.settings.ASSERT && ret != isANOld(dom_id))throw new
		// NullPointerException("Unsure if dominion or not " + dom_id);
		return ret;
	}
	...
	public static boolean isFortress(int fort) {
		...
		if (d == null) {
			TU.log.w("NULL == DOM ||| UPDATE XMLS?!?! ||| " + fort, "DATA");
			return false;
		}
		boolean ret = d.category == CardCategory.FORTRESS_DEFENSE || d.category == CardCategory.FORTRESS_SIEGE
				|| d.category == CardCategory.FORTRESS_CONQUEST;
		return ret;
	}

	/*
	 * public static <T> int getCount(T[] ids, T id) { int c = 0; for(T i :
	 * ids)if(i.equals(id))c++; return c; }
	 */
	...
	/*
	 * public static boolean isRareCommander(Card c) { return
	 * c.getName().equals("Ascaris") ||c.getName().equals("Malika")
	 * ||c.getName().equals("Terrogor") ||c.getName().equals("Maion")
	 * ||c.getName().equals("Cyrus"); }
	 */
	...
	{
		TU.log.d("Queeing Gauntlet", gaunt_name);
		Task.start(() -> {
			synchronized (c) {
				TU.log.d("EOQ Gauntlet", gaunt_name);
				try {
					if (!c.exists()) {

						if (c_tmp.exists()) {
							TU.log.e("Interruption in saving deck, tring to restore it", gaunt_name, "saveGauntletRaw");
							c_tmp.renameTo(c);
						} else {
							TU.log.e("Interruption in saving deck, manual restore needed", gaunt_name,
									"saveGauntletRaw");
							GlobalData.createFile("data/customdecks.txt"); // Does it work?
						}
					}
					...
					c_tmp.renameTo(c);
					// Thread.sleep(1000);
					TU.log.d("Saved Gauntlet", gaunt_name);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public static void saveGauntlet(String gaunt_name, String deck, int[] ids) {
		/*
		 * if(ids[1]==ids[2] && isAN(ids[1])) { int[] tmp = ids.clone(); tmp[2]=0; deck
		 * = Data.getDeckString(tmp); }
		 */
		if (ids[0] == ids[1])
			deck = deck.substring(deck.indexOf(",") + 1);
	...
	public static void appendLine(String file, String line) {
		FileIO.appendLine(file, line);
	}
	@Deprecated
	public static void appendLines(String file, String[] lines) {
		FileIO.appendLines(file, lines);
	}
}
